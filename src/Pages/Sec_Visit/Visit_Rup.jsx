...
function Visit_Rup() {
  ...
  const [sclbookings, setSclBookings] = useState([]);
  const [unibookings, setUniBookings] = useState([]);
  ...
  const fetchBookings = async () => {
    if (userRole === 'security') {
      ...
      .then((response) => {
        ...
        setSclBookings(response.data);
        setLoading(false);
      })
      .get("")
      .then((response) => {
        console.log(response.data);
        setUniBookings(response.data);
        setLoading(false);
      })
      .catch((error) => {
        ...
        <h3>School</h3>
          <Grid width={"100%"} >
            {loading ? (
              ...
              sclbookings.map((booking, index) => (
                <Accordion expanded={expanded === 'panel1'} onChange={handleChange('panel1')}>
                <AccordionSummary aria-controls='panel1d-content' id='panel1d-header'>
                  <Typography><b>{booking.shoolname}</b></Typography>
                </AccordionSummary>
                <AccordionDetails>
                  <Typography>
                    <table>
                      ...
                      <tr>
                        <td>Authorized Person's Name:</td>
                        <td>{booking.Rname}</td>
                      </tr>
                      <tr>
                        <td>Phone No:</td>
                      ...
                      <tr>
                        ...
                        <td>{booking.Mstudent}</td>
                        {/* <td>{booking.Mstudent}+{booking.Fstudent}</td> */}
                      </tr>
                      <tr>
                        <td>No of teachers:</td>
                        <td>{booking.noteachers}</td>
                      </tr>
                      <tr>
                        <td>No of parents:</td>
                        <td>{booking.noparents}</td>
                      </tr>
		                  <tr>
                        <td>Time In:</td>
          ...
          {loading ? (
              ...
              unibookings.map((booking, index) => (
                <Accordion expanded={expanded === 'panel1'} onChange={handleChange('panel1')}>
                <AccordionSummary aria-controls='panel1d-content' id='panel1d-header'>
                  <Typography><b>{booking.uniname}</b></Typography>
                </AccordionSummary>
                <AccordionDetails>
                  <Typography>
                    <table>
                      <tr>
                        <td>University/Coolege Name:</td>
                        <td> {booking.uniname} </td>
                      </tr>
                      <tr>
                        <td>Authorized Person's Name:</td>
                        <td>{booking.Rname}</td>
